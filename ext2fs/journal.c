...
/* Fast-hash mixer to prevent catastrophic libihash clustering on sequential blocks */
static hurd_ihash_key_t
jbd2_mix_hash (const void *key)
{
  uint64_t h = (uint64_t) (uintptr_t) key;

  /* Apply the Avalanche mixer */
  h ^= h >> 23;
  h *= 0x2127599bf4325c37ULL;
  h ^= h >> 47;

  return (hurd_ihash_key_t) h;
}

static int
jbd2_mix_cmp (const void *a, const void *b)
{
  /* We can compare the "pointers" directly, because they ARE the block
   * numbers. */
  return a == b;
}
...
diskfs_journal_start_transaction (void)
{
  ...
  if (txn)
      ...
      if (!txn)
	{
      ...
      hurd_ihash_set_gki (&txn->t_buffer_map, jbd2_mix_hash, jbd2_mix_cmp);
