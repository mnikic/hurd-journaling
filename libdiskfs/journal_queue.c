/* journal_queue.c - Asynchronous queue for the journaling system
...
journal_flusher_thread (void *arg)
{
  while (1)
    {
      while (!journal_enabled && !shutdown_in_progress)
	{
	...
	JOURNAL_FLUSH_TIMEOUT_MS / 1000 + deadline.tv_nsec / 1000000000;
      ...
      /* Wait until queue has enough entries or flush timeout expires. */
      while (count < JOURNAL_QUEUE_MAX && !shutdown_in_progress)
	{
      ...
      // If we are disabled, don't flush!
      if (!journal_enabled)
	{
	  pthread_mutex_unlock (&queue_lock);
	  continue;
