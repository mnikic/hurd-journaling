...
void
diskfs_cached_node_nput_dead (struct node *np)
{
  if (!np)
    return;
  ...
  /* Convert our hard ref -> weak so we will NOT run _diskfs_lastref. */
  refcounts_demote (&np->refcounts, &r);

  /* Proactively drop soft refs while holding the lock. */
  diskfs_try_dropping_softrefs (np);

  /* Now drop our weak ref. */
  refcounts_deref_weak (&np->refcounts, &r);
  JOURNAL_LOG_DEBUG ("Ref count for inode %" PRIu64 " is weak: %u, hard:%u",
		     np->dn_stat.st_ino, r.weak, r.hard);
  if (r.hard == 0 && r.weak == 0)
    diskfs_drop_node (np);
  else
